...
def input_grammar():
    ...
    while True:
        ...
        if all(nt.isalnum() for nt in non_terminals):  # Check for alphanumeric non-terminals
            break
        else:
...
# Function to check if the grammar is simple
def is_simple_grammar(grammar):
    for nt, rules in grammar.items():
        for rule in rules:
            if re.search(r"[A-Z].*[A-Z]", rule): # Check for more than one Non-Terminal
                return False
    return True

# Function to parse the sequence and check if it is accepted
def parse_sequence(grammar, start_symbol, sequence):
    def parse(symbol, seq):
        ...
        if not seq or not symbol:
            return False
        if symbol[0].islower() or symbol[0].isdigit(): # Terminal symbol
            if seq[0] == symbol[0]:
